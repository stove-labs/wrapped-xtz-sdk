...
import { checkIntegrity, fetchContractCode, packMichelson, sha256 } from './WXTZHelpers';
...
test.skip('checks validity for valid code', async (t) => {
  const hasIntegrity = await checkIntegrity(Constants.deployments.delphinet.core.checksum, ovenContractCodeMicheline);
  t.is(hasIntegrity, true);
});

test('checks validity for invalid code', async (t) => {
  const hasIntegrity = await checkIntegrity(Constants.deployments.delphinet.oven.checksum, 'invalid code');
